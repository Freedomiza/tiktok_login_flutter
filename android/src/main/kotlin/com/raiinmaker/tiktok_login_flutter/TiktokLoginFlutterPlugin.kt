...
class TiktokLoginFlutterPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  private fun initializeTiktokLogin(call: MethodCall, result: MethodChannel.Result) {
    try {
      ...
      authApi = activity?.let { AuthApi(activity = it) }!!
      result.success(true)
    } catch (e: Exception) {
      result.error("INITIALIZATION_FAILED", e.localizedMessage, null)
    }
  }
  ...
  private fun authorize(call: MethodCall, result: MethodChannel.Result) {
    ...
    try {
      ...
      // "com.raiinmaker.tiktok_login_flutter.tiktokapi.TikTokEntryActivity"
      //        tiktokOpenApi.authorize(request);

    } catch (e: Exception) {
      result.error("AUTHORIZATION_REQUEST_FAILED", e.localizedMessage, null)
    }
  }
